...
public class DeleteAssignment extends AsyncTask<AssignmentEntity, Void, Void> {
    private Application application;
    private OnAsyncEventListener callback;
    private Exception exception;

    public DeleteAssignment(Application application, OnAsyncEventListener callback) {
    ...
    protected Void doInBackground(AssignmentEntity... params) {
        try {
            for (AssignmentEntity assignment: params)
                ((BaseApp) application).getDatabase().assignmentDao().delete(assignment);
        } catch (Exception e){
            exception=e;
        }
        return null;
